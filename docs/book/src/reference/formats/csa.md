...
rsshogi の読み込みは宣言行に依存せず、UTF-8 として解釈を試みて失敗したら Shift_JIS
へフォールバックします。宣言行自体は読み飛ばします。書き出しは V3.0 を指定したときだけ
宣言行を出力し、その内容は実際の出力エンコーディングと一致します。
...
- ✅ 手番行前後の `'*...` を `initial_comment` として往復
- ✅ `%...` 終局行に続く `T...` / コメント行の保持
- ✅ `$TIME+` / `$TIME-` / `$MAX_MOVES` / `$JISHOGI` の取り込み
- ✅ `$NOTE` の `RecordMetadata::comment` への取り込みと再出力
