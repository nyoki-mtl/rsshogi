...
1.1.0 で `Position::key` の持ち駒の寄与が XOR 合成へ変わり、キーの値そのものが変わりました。
構造は version `1` と同一ですが、格納されているキーの意味が違うため、version を `2` へ上げています。
1.0.x 以前に生成した定跡バイナリは読み込み時に `Unsupported("version mismatch")` で拒否されるので、
現行バージョンで再生成してください。
