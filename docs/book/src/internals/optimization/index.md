...
この crate 自体は探索方策や評価関数を定義しないため、NPS の改善は利用側の探索条件を含めて判断する。
...
ホットパスでのみ、不要な割り当て、繰り返す変換、同じ利きの再計算を減らす。

CPU 固有の intrinsic や新しい表引き方式は、同値フォールバックと測定上の優位がある場合に限る。

`target-cpu=native` は実行したマシン向けの命令を有効にするため、異なる CPU に配布する成果物の既定値にはしない。
...
計測用ベンチマークは原因を絞る道具であり、探索全体の改善を保証する採用判定ではない。
...
性能が良くても、公開 API の結果、局面不変条件、unsafe 契約を変える差分は採用できない。
...
アラインメントは必要な SIMD データ境界を満たすための手段であり、単独で速度改善を保証する設定ではない。

## よくある誤り

`target-cpu=native` を含む結果を、より低い CPU 要件の配布物へ一般化しない。

CPU 固有命令を追加するときは、対応しないビルドの代替経路とテストを同時に維持する。
