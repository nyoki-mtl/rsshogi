...
128 ビットのビットボードに対しては、上位・下位の各 64 ビットに `popcnt` を適用し、結果を加算します。
...
256 ビットレジスタに 2 つの 128 ビットのビットボードを格納し、1 命令で同時に演算できます。
rsshogi では `Bitboard256` がこのパターンを実装しており、[角の 4 方向利き計算](../bitboard/attacks.md#角bishop_attacksの-bitboard256-並列処理)で `_mm256_and_si256`、`_mm256_sub_epi64`、`_mm256_xor_si256` を連鎖させて使用します。
