...
探索エンジンは毎秒数百万から数千万のノードを処理し、そのすべてで指し手を生成します。
つまり指し手生成の速度は、エンジン全体の速度をそのまま決めます。

しかも将棋はチェスより不利です。
駒打ちと成り不成の選択肢があるため、1 局面あたりの合法手はチェスの 2 倍から 3 倍に膨らみます。
...
- **疑似合法手の生成**: 合法性は適用前に `is_legal_after_pseudo_move32()` で確認する
- **段階的な生成**: 良い手を先に返し、早期カットを促す
- **ビットボードの活用**: 候補集合を作ってから `pop_lsb` で一括に列挙する
- **Move Ordering（指し手のソート順序）**: 取り駒と静かな手を分けて生成すると安定する
