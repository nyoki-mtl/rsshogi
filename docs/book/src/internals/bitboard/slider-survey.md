...
したがって他エンジンの方式名や単独の命令レイテンシだけから、rsshogi の最適解を決めることはできない。
...
この章でいう「現行経路」は `lance_attacks`、`bishop_attacks`、`rook_attacks` の実装を指し、private なレイのメモリレイアウトを外部契約にはしない。
...
単独関数の速度が良くても、攻撃テーブルの常駐量や局面更新のコストを含めた手生成全体で退行すれば採用しない。

PEXT や Magic の導入は、対応 CPU と生成物を増やすため、根拠となる全体測定と保守上の必要性がある場合に限る。

このリポジトリでは、ベンチマークの結果を正確性テストの代替にしない。
...
| --- | --- | --- | --- |
| 実装の直接性 | 高い。 | 中程度。 | 中程度。 |
| 追加データ | ほぼ不要。 | 利き表と添字用データが必要。 | 方向レイが必要。 |
| CPU 固有命令 | 不要。 | PEXT 方式では必要。 | SIMD は任意の並列化。 |
| 基準実装への適性 | 高い。 | 低い。 | 中程度。 |
| 保守時の要点 | 盤端と遮蔽。 | 表生成と添字。 | ビット順と反転方向。 |

この表は一般的な設計比較であり、特定 CPU での速度順位を約束しない。
