...
利きの計算は、駒によって難しさがまったく違います。
歩や桂の利きは相手の駒がどこにあっても変わらないので、マスごとに答えを持っておけば済みます。
一方、香や角や飛の利きは、途中に駒があればそこで止まります。
81 マス分の表では足りず、盤上の駒の配置ごとに答えが変わってしまいます。
この差をどう埋めるかが、このページの中心です。
