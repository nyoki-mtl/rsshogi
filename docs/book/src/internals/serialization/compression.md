...
局面を SFEN で書けば 1 局面あたり 80 バイト前後です。
学習データとして 1 億局面を貯めると、それだけで 8 GB になります。
将棋の合法局面は多く見積もっても 10⁷¹ 通りなので、情報量としては 30 バイトで足りるはずです。
この差はどこから来るのか、そしてどこまで詰められるのかを追います。
...
Position が備える特性のうち、圧縮と衝突しやすいのが**差分更新**（incremental update）の高速性です。
