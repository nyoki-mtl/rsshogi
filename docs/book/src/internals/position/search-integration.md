...
`Position` が持つのは、ルールを正しく判定するために必要な状態だけです。
NNUE の差分情報や評価用の補助ハッシュは、`Position` の外、つまり探索エンジン側に置きます。

境界をここに引いているのは、3 つの理由からです。

1. **責務の分離**: 盤面管理と評価差分更新は別の関心事。コアに評価ロジックが混ざると、評価関数を使わない用途（GUI、棋譜解析ツールなど）にもその分の重さが乗る
2. **評価方式の自由度**: NNUE、駒割り、手作り評価と、採る方式はエンジンごとに違う。コアが特定の方式を前提にすると切り替えが難しくなる
3. **コスト**: 使わないフィールドでも、初期化とコピーと差分更新のコストはかかる
