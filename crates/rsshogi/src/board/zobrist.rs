...
//! # YaneuraOu との関係
//!
//! テーブル生成 scheme は YaneuraOu と同じ（固定シード Xorshift64\*、seed 20151225）だが、
//! **キー値には互換性がない**。YaneuraOu は `side = 1` 固定・全キー `& ~1ULL` として
//! bit 0 に手番を埋め込むが、rsshogi はこれを行わない（`side` は通常の乱数定数）。
//! YaneuraOu の置換表や book とキーを突き合わせることはできない。
//!
//! # ビット幅
//!
